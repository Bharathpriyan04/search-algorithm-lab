...
def search():
    ...
    if not text or not pattern:
        return jsonify({"error": "Text and pattern cannot be empty."}), 400
    if len(pattern) > len(text):
        return jsonify({"error": "Pattern cannot be longer than the text."}), 400
    ...
    return jsonify({
        ...
        "rabin":  {"matches": m3, "comparisons": c3},
    })
...
def performance():
    ...
    for p in patterns:
        ...
        results.append({"pattern": p, "naive": c1, "kmp": c2, "rabin": c3})
